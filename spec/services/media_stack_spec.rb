...
describe MediaStack do
  describe "::get_one_hundred_articles", vcr: { re_record_interval: 7.days } do
    it "returns empty array if no results found" do
      articles = MediaStack.get_one_hundred_articles(["apoinwfimdsoihqpsldfjeiwowjfpaosidfj"], 0)
    ...
    it "returns array of hashes that can be used to create a new article after associating them with a topic" do
      articles = MediaStack.get_one_hundred_articles(["metaverse facebook"], 0)
      user = Fabricate(:user)
    ...
    it "logs each request in the media_stack_queries table" do
      MediaStack.get_one_hundred_articles(["metaverse facebook"], 0)
      MediaStack.get_one_hundred_articles(["U.S. president"], 0)
